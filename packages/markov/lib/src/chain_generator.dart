...
class MarkovChainGenerator implements StreamConsumer<String> {
  ...
  final MarkovChain _chain;

  final Queue<String> _stringTokens;
  ...
  Future addStream(Stream<String> stream) => stream.forEach((line) async {
        ...
        while (_stringTokens.length >= order + 1) {
          final precedent =
              TokenSequence(_stringTokens.take(order).map(Token.new));
  ...
  Iterable<String> _tokenizeLine(String line) sync* {
    ...
    while (!scanner.isDone) {
      if (scanner.scan(_link) ||
        ...
        yield scanner.lastMatch!.group(0)!;
      }
